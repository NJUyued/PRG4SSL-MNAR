    ...
    def get_data(self):
        ...
        if self.name=='stl10':
            if self.train:
                ...
                dset_lb = dset(self.data_dir, split='train', folds=0, download=True)
                data_lb, targets_lb = dset_lb.data, dset_lb.labels
#                 dset_lb_ulb = dset(self.data_dir, split='train+unlabeled', download=True)
                dset_lb_ulb = dset(self.data_dir, split='train+unlabeled', folds=0, download=True)
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        if self.name=='stl10':
            ...
            transform = self.transform
            data_dir = self.data_dir
            # lb_data, lb_targets, ulb_data, ulb_targets = split_ssl_data(data_lb, targets_lb, 
            ...
            # eval_ulb_dset = BasicDataset(ulb_data, ulb_targets, num_classes, 
            #                     transform, use_strong_transform, strong_transform, onehot)
