...
def split_ssl_data(dataset, data, target, num_labels, num_classes, save_path, index=None, include_lb_to_ulb=True, mismatch='none', n0=0, gamma=0):
    """
    ...
    lb_data, lbs, lb_idx, distri_lb = sample_labeled_data(data, target, num_labels, num_classes, save_path, index, mismatch, n0, gamma, dataset)
    ...
    if mismatch=='cadr' and n0!=0:   
        assert n0<=200 and n0>0  
        data =  data[ulb_idx]
        target =  target[ulb_idx]
        all = 0      
        for c in range(num_classes):
            idx = np.where(target == c)[0]
            lt_len = math.ceil(dic[dataset] * math.pow(n0,-((nc-c)/nc)))
            if lt_len<len(idx):
                idx = np.random.choice(idx, lt_len, False)    
                samply_dist_u.append(lt_len)
            else:
...
def sample_labeled_data(data, target, 
    ...
    elif mismatch=='darp' or mismatch=='da r p_reversed':
