...
from .fixmatch_utils import consistency_loss, consistency_loss_dis_3, consistency_loss4, Get_Scalar, Get_Scalar_d
...
class FixMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u,\
        ...
        self.p_fn = Get_Scalar_d(p_cutoff) #confidence cutoff function
    ...
    def train(self, args, logger=None):
        ...
        feature_extractor = self.train_model.module.feature_extractor.train(True)
        cls_reverse = self.train_model.module.classifier_reverse.train(True)
        ...
        for (x_lb, y_lb), (x_ulb_w, x_ulb_s, target , idx) in zip(self.loader_dict['train_lb'], self.loader_dict['train_ulb']):
            ...
            tb_dict['train/bank_radio'] = count/len(idx)
            tb_dict['train/mask_ratio'] = 1.0 - mask.detach() 
            tb_dict['lr'] = self.optimizer.param_groups[0]['lr']
            tb_dict['lamda_d'] = lambda_d
    ...
    def evaluate(self, eval_loader=None, args=None , lb_loader=None ,ulb_loader=None,p_cutoff=None):
        ...
        eval_model.eval()
        feature_extractor = self.eval_model.module.feature_extractor
        cls_reverse=self.eval_model.module.classifier_reverse
        ...
        for x, y in eval_loader:
            ...
            acc_fix = torch.sum(max_idx_fix == y)
        ...
        return {'eval/loss': total_loss/total_num, 'eval/top-1-acc': total_acc/total_num , 'eval/top-1-acc(fix)': total_acc_fix/total_num , 
