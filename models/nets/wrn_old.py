...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, bn_momentum=0.1, leaky_slope=0.0, dropRate=0.0):
        ...
        self.conv1 = nn.Conv2d(3, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.conv2 = nn.Conv2d(3, 32, kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
        ...
        self.bn2 = nn.BatchNorm2d(nChannels[1], momentum=bn_momentum)
        ...
        # self.fc3 = nn.Linear(nChannels[1], num_classes)
        self.nChannels1 = nChannels[3]
        self.nChannels2 = nChannels[2]
        self.nChannels3 = nChannels[1]
    ...
    def forward(self, x, ood_test=False, sel=3):
        if sel == 3:
            # print('3-------------')
            # print(x.size())
            out = self.conv1(x)
            ...
            # print(out.size())
            out = out.view(-1, self.nChannels1)
            # print(out.size())
            output = self.fc(out)
            # print(output.size())
            if ood_test:
